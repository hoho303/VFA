...
class MetaSupervisedContrastiveLoss(nn.Module):
    ...
    def forward(self,
        ...
        Args:
            query_features (tensor): Shape of (N, K) where N is the number
                of features to be compared and K is the channels.
            support_features (tensor): Shape of (M, K) where M is the number
                of features to be compared and K is the channels.
            query_labels (tensor): Shape of (N).
            support_labels (): Shape of (M).
        ...
        assert query_features.shape[0] == query_labels.shape[0] == ious.shape[0]
        assert support_features.shape[0] == support_labels.shape[0]

        if len(query_labels.shape) == 1:
            query_labels = query_labels.reshape(-1, 1)

        features = torch.cat([query_features, support_features], dim=0)
        labels = torch.cat([query_labels, support_labels], dim=0)

        pseudo_ious = torch.ones_like(support_labels)
        ious = torch.cat([ious, pseudo_ious], dim=0)
